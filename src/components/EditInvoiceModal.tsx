...
export function EditInvoiceModal({ invoice, onClose, requireOtp, exchangeMode }: EditInvoiceModalProps) {
  ...
  // نفس المنتج ممكن يتكرر في الفاتورة بموظفين مختلفين، فالتفتيش بـ product.id
  // لوحده بيعدّل السطرين مع بعض. lineKey بيميّز كل سطر.
  const lineKey = (item: OrderItem) => item.order_item_id || `${item.id}::${item.salesperson_id || 'none'}`;

  const handleAddProduct = (product: Product) => {
    setCart(prev => {
      // الصنف الجديد بيتضاف بلا موظف مسؤول — الأدمن بيحدده من «مبيعات اليوم».
      const existing = prev.find(p => !p.order_item_id && p.id === product.id && !p.salesperson_id);
      if (existing) {
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            ...
            <table className="w-full text-sm text-right">
              ...
              <tbody className="divide-y divide-slate-100">
                {cart.map(item => (
                  <tr key={lineKey(item)} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4 font-medium text-slate-800">{item.name}</td>
                    <td className="p-4">
                      <div className="flex items-center justify-center gap-3">
                        <button onClick={() => handleUpdateQuantity(lineKey(item), -1)} className="p-1 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded">
                          <Minus size={16} />
                        </button>
                    ...
                    <td className="p-4">
                      <input
                        ...
                        onChange={(e) => handleUpdatePrice(lineKey(item), Number(e.target.value))}
                    ...
                    <td className="p-4 text-left">
                      <button onClick={() => handleRemoveItem(lineKey(item))} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                        <Trash2 size={18} />
                      </button>
                    </td>
                  </tr>
