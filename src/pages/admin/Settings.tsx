  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-4 md:p-8 rounded-3xl shadow-sm border border-slate-100 space-y-6">
        ...
        {/* ── عمولات الموظفين ── */}
        <div className="pt-6 border-t border-slate-100">
          <h2 className="text-lg font-black text-slate-800 mb-1">عمولات الموظفين</h2>
          <p className="text-slate-500 text-sm mb-4">
            كل صنف في الفاتورة له موظف مسؤول بيتحدد من الكاشير. الخدمات بتاخد نسبة تلقائية من قيمتها،
            والمنتجات بتاخد قيمة يدوية بتكتبها من صفحة «مبيعات اليوم».
          </p>
          <label className="flex items-center justify-between gap-3 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
            <div>
              <span className="block text-sm font-bold text-emerald-800">نسبة عمولة الخدمات %</span>
              <span className="block text-[11px] font-bold text-emerald-600 mt-0.5">
                مثال: خدمات بـ 10,000 {formData.currency || 'ج.م'} في الشهر ← حافز {(10000 * (Number(formData.serviceCommissionRate ?? DEFAULT_SERVICE_COMMISSION_RATE) || 0) / 100).toLocaleString()} {formData.currency || 'ج.م'}
              </span>
            </div>
            <div className="flex items-center gap-1 shrink-0">
              <input
                ...
                step="0.5"
                dir="ltr"
                value={formData.serviceCommissionRate ?? DEFAULT_SERVICE_COMMISSION_RATE}
                onChange={(e) => setFormData({ ...formData, serviceCommissionRate: e.target.value === '' ? undefined : Math.min(100, Math.max(0, parseFloat(e.target.value) || 0)) })}
                className="w-20 bg-white border border-emerald-200 rounded-lg px-2 py-2 text-center font-black text-emerald-700"
              />
