...
import { useStore, type Employee, type EmployeeTransaction, type EmployeeLeave, DEFAULT_SERVICE_COMMISSION_RATE } from '../../store/useStore';
import { computeEmployeeSales, ordersInMonth, netQuantity, localMonthKey, localDayKey, emptySalesStats, type EmployeeSalesStats } from '../../utils/commissions';
import {
...
export default function Employees() {
  const {
    employees, employeeTransactions, employeeLeaves, attendance, storeSettings, orders,
  ...
  // نسبة عمولة الخدمات — إعداد عام من صفحة الإعدادات
  const serviceRate = storeSettings.serviceCommissionRate ?? DEFAULT_SERVICE_COMMISSION_RATE;

  // مبيعات وعمولات الموظف في شهر (YYYY-MM بالتوقيت المحلي).
  // كل صنف بينتسب لمسؤوله المسجّل عليه — مفيش قسمة بالتساوي زي الأول.
  const employeeMonthStats = (emp: any, month: string): EmployeeSalesStats & { profit: number; sales: number } => {
    const monthOrders = ordersInMonth(orders as any, month);
    const stats = computeEmployeeSales(monthOrders, emp?.id, serviceRate);
    // الربح المحقق من أصناف الموظف بس (مش الفاتورة كلها)
    let profit = 0;
    for (const o of monthOrders) {
      for (const it of o.items || []) {
        if (it.salesperson_id !== emp?.id) continue;
        const qty = netQuantity(it);
        const cost = Number(it.average_purchase_price ?? it.purchase_price) || 0;
        profit += ((Number(it.sale_price) || 0) - cost) * qty;
      }
    }
    return { ...stats, profit, sales: stats.totalSales };
  };
  ...
  // قيمة العمولة اللي اتضافت للراتب الحالي (0 = لسه ما اتضافتش).
  // بتمنع الإضافة مرتين، وبتتصفّر لما الصافي يتحسب من الأول.
  const [commissionAdded, setCommissionAdded] = useState(0);
  ...
  // بتشيل سطر العمولة من الملاحظة لما المبلغ يتحسب من الأول (وإلا الملاحظة
  // بتفضل بتقول "+ عمولة" والمبلغ مفيهوش عمولة).
  const stripCommissionNote = (note: string, month: string) =>
    note
      .replace(new RegExp(`\\s*\\+?\\s*عمولة مبيعات شهر ${month}: [\\d.]+`), '')
      .replace(/^\s*\+\s*/, '')
      .trim();
  ...
  const exportPayroll = () => {
    const rows = employees.map((emp) => {
      ...
      return {
        ...
        'المتبقي': s.remaining,
        'مبيعات خدمات': Math.round(sales.serviceSales),
        'مبيعات منتجات': Math.round(sales.productSales),
        'إجمالي مبيعاته': Math.round(sales.totalSales),
        [`حافز الخدمات (${serviceRate}%)`]: Number(sales.serviceCommission.toFixed(2)),
        'عمولة المنتجات': Number(sales.productCommission.toFixed(2)),
        'إجمالي العمولة': Number(sales.totalCommission.toFixed(2)),
        'أرباحه للشركة': Math.round(sales.profit),
      };
    });
  ...
  // الفترة المختارة في البروفايل — مصدر واحد بيحكم المعاملات والإجازات
  // والحضور والمبيعات. قبل كده كل قسم كان بيفلتر بطريقته، وفلتر الشهر كان
  // متجاهَل أصلاً في الكروت.
  //
  // الحقول الفاضية بترجع لقيمة افتراضية: input type=month/number ينفع يتفضّى،
  // و "أي نص".startsWith('') = true، فكان تفضية الحقل بتعرض كل الحركات
  // بدل ما تعرض الشهر المطلوب.
  const profilePeriod = useMemo(() => {
    const todayKey = localDayKey(new Date());
    const thisMonth = todayKey.slice(0, 7);
    switch (profileTimeFilter) {
      case 'custom_month':
        return { kind: 'month' as const, month: /^\d{4}-\d{2}$/.test(profileCustomMonth) ? profileCustomMonth : thisMonth };
      case 'custom_year':
        return { kind: 'year' as const, year: /^\d{4}$/.test(profileCustomYear) ? profileCustomYear : String(new Date().getFullYear()) };
      case 'week': {
        const weekStart = new Date();
        weekStart.setDate(weekStart.getDate() - 6);
        return { kind: 'range' as const, from: localDayKey(weekStart), to: todayKey };
      }
      case 'all':
        return { kind: 'all' as const };
      default:
        return { kind: 'month' as const, month: thisMonth };
    }
  }, [profileTimeFilter, profileCustomMonth, profileCustomYear]);

  // monthKey = فترة المحاسبة (الشهر اللي الحركة بتخصّه)، dayKey = تاريخ التنفيذ.
  // مهم نفرّق: راتب شهر 6 بيتصرف يوم 5/7 — بـ OR بين الاتنين (زي الأول) كان
  // بيظهر تحت شهر 6 وشهر 7، فتبديل الفلتر ما كانش بيغيّر حاجة.
  const inProfilePeriod = useCallback((monthKey: string, dayKey: string) => {
    switch (profilePeriod.kind) {
      case 'month': return monthKey === profilePeriod.month;
      case 'year': return monthKey.startsWith(profilePeriod.year);
      case 'range': return dayKey >= profilePeriod.from && dayKey <= profilePeriod.to;
      default: return true;
    }
  }, [profilePeriod]);
  ...
  const profileTransactions = useMemo(() => {
    ...
    return employeeTransactions
      .filter(t => t.employee_id === profileEmployee.id)
      .filter(t => inProfilePeriod(t.month || localMonthKey(t.created_at), localDayKey(t.created_at)))
      .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  }, [profileEmployee, employeeTransactions, inProfilePeriod]);

  const profileLeaves = useMemo(() => {
    if (!profileEmployee) return [];
    return employeeLeaves
      .filter(l => l.employee_id === profileEmployee.id)
      .filter(l => inProfilePeriod(l.month || localMonthKey(l.start_date), l.start_date))
      .sort((a, b) => new Date(b.start_date).getTime() - new Date(a.start_date).getTime());
  }, [profileEmployee, employeeLeaves, inProfilePeriod]);

  const profilePeriodLabel = useMemo(() => {
    switch (profilePeriod.kind) {
      case 'month': return `شهر ${profilePeriod.month}`;
      case 'year': return `سنة ${profilePeriod.year}`;
      case 'range': return `${profilePeriod.from} → ${profilePeriod.to}`;
      default: return 'كل الفترات';
    }
  }, [profilePeriod]);

  // مبيعات وعمولات الموظف في نفس الفترة المختارة
  const profileSales = useMemo(() => {
    if (!profileEmployee) return emptySalesStats();
    const periodOrders = (orders as any[]).filter((o) =>
      !o.is_deleted && o.type === 'sale' && inProfilePeriod(localMonthKey(o.date), localDayKey(o.date))
    );
    return computeEmployeeSales(periodOrders as any, profileEmployee.id, serviceRate);
  }, [profileEmployee, orders, inProfilePeriod, serviceRate]);
  ...
  const profileAttendance = useMemo(() => {
    ...
    // مشتقّة من نفس الفترة الموحّدة — قبل كده لو حقل الشهر اتفضّى كان
    // بيطلع end = "-NaN" فتاريخ غير صالح، والحلقة ما تلفّش ولا مرة والجدول
    // يفضل فاضي من غير أي رسالة خطأ.
    let start: string, end: string;
    if (profilePeriod.kind === 'range') {
      start = profilePeriod.from; end = profilePeriod.to;
    } else if (profilePeriod.kind === 'month') {
      const [y, m] = profilePeriod.month.split('-').map(Number);
      const last = new Date(y, m, 0).getDate();
      start = `${profilePeriod.month}-01`;
      end = `${profilePeriod.month}-${String(last).padStart(2, '0')}`;
    } else if (profilePeriod.kind === 'year') {
      start = `${profilePeriod.year}-01-01`; end = `${profilePeriod.year}-12-31`;
    } else { // all
  ...
  }, [profileEmployee, attendance, employeeLeaves, profilePeriod]);
  ...
  const handleOpenTransModal = (emp: Employee, type: 'salary' | 'advance' | 'incentive', transaction?: EmployeeTransaction) => {
    ...
    // المبلغ المحفوظ في حركة قديمة أصلاً شامل أي عمولة اتضافت وقتها
    setCommissionAdded(0);
    ...
    // الشهر بيتاخد من فلتر الشهر اللي فوق (أو الشهر المفتوح في البروفايل)
    // عشان لو الأدمن اختار شهر 6 وضغط "صرف راتب" يلاقي أرقام شهر 6 قدامه.
    const currentMonth = selectedProfileId && profilePeriod.kind === 'month' ? profilePeriod.month : payrollMonth;
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto h-[calc(100vh-2rem)] overflow-y-auto" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 bg-white p-6 rounded-[32px] shadow-sm border border-slate-100">
        ...
        {/* فلاتر القائمة — بتتخفي داخل البروفايل لأنها بتفلتر قائمة الموظفين
            مش محتوى البروفايل. كانت ظاهرة وشغّالة على حاجة مش على الشاشة،
            فالمستخدم يكتب في البحث وما يحصلش حاجة. */}
        {!selectedProfileId && (
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl px-3 py-2">
            <input type="month" value={payrollMonth} onChange={(e) => setPayrollMonth(e.target.value || localMonthKey(new Date()))} className="bg-transparent text-sm font-bold outline-none" />
      ...
      {selectedProfileId && profileEmployee ? (
        <div className="space-y-6">
          ...
          {/* Profile Stats */}
          {/* ── المبيعات والعمولات (نفس الفترة المختارة تحت) ── */}
          <div className="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden">
            <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-wrap items-center justify-between gap-2">
              <h3 className="text-lg font-black text-slate-800 flex items-center gap-2">
                ...
                المبيعات والعمولات
                <span className="text-[11px] font-bold text-slate-400">({profilePeriodLabel})</span>
              </h3>
              <div className="flex items-center gap-2">
                {profileSales.pendingProductLines > 0 && (
                  <span className="text-[11px] font-black bg-amber-50 text-amber-700 border border-amber-100 px-3 py-1.5 rounded-lg">
                    {profileSales.pendingProductLines} صنف لسه محتاج تأكيد عمولة من «مبيعات اليوم»
                  </span>
                )}
                <span className="text-[11px] font-black bg-indigo-50 text-indigo-600 border border-indigo-100 px-3 py-1.5 rounded-lg">
                  عمولة الخدمات {serviceRate}%
                </span>
              </div>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-px bg-slate-100">
              <div className="bg-white p-5 flex flex-col justify-center">
                <span className="text-slate-400 font-bold text-xs mb-1">مبيعات خدمات</span>
                <span className="text-xl font-black text-slate-800">{Math.round(profileSales.serviceSales).toLocaleString()} <span className="text-[11px] font-medium text-slate-400">{storeSettings.currency}</span></span>
                <span className="text-[10px] font-bold text-slate-400 mt-1">{profileSales.serviceCount} خدمة</span>
              </div>
              <div className="bg-white p-5 flex flex-col justify-center">
                <span className="text-slate-400 font-bold text-xs mb-1">مبيعات منتجات</span>
                <span className="text-xl font-black text-slate-800">{Math.round(profileSales.productSales).toLocaleString()} <span className="text-[11px] font-medium text-slate-400">{storeSettings.currency}</span></span>
              </div>
              <div className="bg-white p-5 flex flex-col justify-center">
                <span className="text-emerald-500 font-bold text-xs mb-1">حافز الخدمات ({serviceRate}%)</span>
                <span className="text-xl font-black text-emerald-600">{Math.round(profileSales.serviceCommission).toLocaleString()} <span className="text-[11px] font-medium text-emerald-400">{storeSettings.currency}</span></span>
              </div>
              <div className="bg-white p-5 flex flex-col justify-center">
                <span className="text-emerald-500 font-bold text-xs mb-1">عمولة المنتجات</span>
                <span className="text-xl font-black text-emerald-600">{Math.round(profileSales.productCommission).toLocaleString()} <span className="text-[11px] font-medium text-emerald-400">{storeSettings.currency}</span></span>
              </div>
              <div className="bg-indigo-50 p-5 flex flex-col justify-center col-span-2 lg:col-span-1">
                <span className="text-indigo-500 font-bold text-xs mb-1">إجمالي المبيعات • العمولة</span>
                <span className="text-xl font-black text-indigo-700">{Math.round(profileSales.totalSales).toLocaleString()}</span>
                <span className="text-lg font-black text-indigo-600">← {Math.round(profileSales.totalCommission).toLocaleString()} <span className="text-[11px] font-medium text-indigo-400">{storeSettings.currency}</span></span>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'employees' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEmployees.map(emp => {
            // بيتبع فلتر الشهر اللي فوق. قبل كده كان متثبّت على شهر النهاردة،
            // فالمستخدم يغيّر الشهر وما يتحركش رقم على الشاشة — الشهر المختار
            // كان بيروح لملف الإكسل بس.
            const stats = getEmployeeMonthStats(emp.id, payrollMonth);
            const sales = employeeMonthStats(emp, payrollMonth);
            ...
            return (
              <div key={emp.id} className={`bg-white rounded-[32px] p-6 shadow-sm border transition-all group ${isActive ? 'border-slate-100 hover:border-indigo-200' : 'border-slate-200 opacity-75 grayscale-[0.25]'}`}>
                ...
                <div className="space-y-3 mb-6">
                  ...
                  {sales.totalSales > 0 && (
                    <div className="flex items-center justify-between text-sm p-3 bg-indigo-50 rounded-xl border border-indigo-100 mt-2">
                      <span className="text-indigo-600 font-bold flex items-center gap-1">
                        مبيعاته • عمولته
                        {sales.pendingProductLines > 0 && (
                          <span className="text-[9px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-md" title="فيه منتجات لسه محتاجة تأكيد عمولة من مبيعات اليوم">
                            {sales.pendingProductLines} معلّق
                          </span>
                        )}
                      </span>
                      <span className="font-black text-indigo-700">
                        {Math.round(sales.totalSales).toLocaleString()} • {Math.round(sales.totalCommission).toLocaleString()} {storeSettings.currency}
                      </span>
                    </div>
                  )}
                  <div className="flex items-center justify-between text-sm p-3 bg-emerald-50 rounded-xl border border-emerald-100 mt-2">
                    <span className="text-emerald-600 font-bold flex items-center gap-1">المتبقي صرفه ({payrollMonth})</span>
                      ...
                      <input 
                        ...
                        onChange={e => {
                          ...
                          setTransFormData({
                            ...
                            paid_visa: '', paid_wallet: '', paid_instapay: '', paid_method5: '', paid_method6: '',
                            note: stripCommissionNote(transFormData.note, transFormData.month),
                          });
                          setCommissionAdded(0);
                        }} 
                      />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    onChange={e => {
                      ...
                      if (!newMonth) return; // input type=month ينفع يتفضّى
                      if (transType === 'salary') {
                        ...
                        setTransFormData({
                          ...
                          paid_cash: net.toFixed(2),
                          paid_visa: '', paid_wallet: '', paid_instapay: '', paid_method5: '', paid_method6: '',
                          // الشهر اتغيّر ⇒ عمولة الشهر القديم مالهاش لازمة
                          note: stripCommissionNote(transFormData.note, transFormData.month),
                        });
                        setCommissionAdded(0);
                      } else {
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={e => setTransFormData({...transFormData, amount: e.target.value, paid_cash: e.target.value, paid_visa: '', paid_wallet: '', paid_instapay: '', paid_method5: '', paid_method6: ''})}
                  />
                </div>
              </div>

              {transType === 'salary' && (() => {
                // العمولة بقت محسوبة من الأصناف نفسها: الخدمات بنسبة الإعدادات،
                // والمنتجات بالقيم اللي الأدمن أكّدها في مبيعات اليوم.
                ...
                return (
                  <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 space-y-2">
                    <div className="flex items-center justify-between text-sm font-black text-emerald-800">
                      <span>عمولة المبيعات — شهر {transFormData.month}</span>
                      <span>إجمالي مبيعاته: {Math.round(stats.totalSales).toLocaleString()} {cur}</span>
                    </div>

                    <div className="grid grid-cols-2 gap-2 text-[11px] font-bold">
                      <div className="bg-white border border-emerald-100 rounded-lg p-2">
                        <div className="text-slate-500">خدمات: {Math.round(stats.serviceSales).toLocaleString()} {cur}</div>
                        <div className="text-emerald-700 font-black">حافز {serviceRate}% = {stats.serviceCommission.toFixed(2)} {cur}</div>
                      </div>
                      <div className="bg-white border border-emerald-100 rounded-lg p-2">
                        <div className="text-slate-500">منتجات: {Math.round(stats.productSales).toLocaleString()} {cur}</div>
                        <div className="text-emerald-700 font-black">عمولة مؤكَّدة = {stats.productCommission.toFixed(2)} {cur}</div>
                      </div>
                    </div>

                    {stats.pendingProductLines > 0 && (
                      <div className="text-[10px] font-bold text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-2">
                        ⚠️ فيه {stats.pendingProductLines} صنف منتجات لسه ما اتأكّدتش عمولته من صفحة «مبيعات اليوم» — مش داخل في الرقم ده.
                      </div>
                    )}

                    <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-emerald-200">
                      <span className="text-sm font-black text-emerald-800">إجمالي العمولة = {commission.toFixed(2)} {cur}</span>
                      {/* الزرار كان بيتضغط أكتر من مرة فيضيف العمولة مرتين من غير
                          ما حد ياخد باله. دلوقتي بيتقفل بعد الإضافة، والتراجع بيشيلها. */}
                      {commissionAdded ? (
                        <button type="button"
                          onClick={() => {
                            setTransFormData({
                              ...transFormData,
                              paid_cash: Math.max(0, (parseFloat(transFormData.paid_cash) || 0) - commissionAdded).toFixed(2),
                              amount: Math.max(0, (parseFloat(transFormData.amount) || 0) - commissionAdded).toFixed(2),
                              note: stripCommissionNote(transFormData.note, transFormData.month),
                            });
                            setCommissionAdded(0);
                          }}
                          className="mr-auto bg-slate-200 hover:bg-slate-300 text-slate-700 text-xs font-bold px-3 py-2 rounded-lg">
                          ✓ اتضافت {commissionAdded.toFixed(2)} — اضغط للتراجع
                        </button>
                      ) : (
                        <button type="button" disabled={commission <= 0}
                          onClick={() => {
                            setTransFormData({
                              ...
                              note: `${transFormData.note}${transFormData.note ? ' + ' : ''}عمولة مبيعات شهر ${transFormData.month}: ${commission.toFixed(2)}`,
                            });
                            setCommissionAdded(commission);
                          }}
