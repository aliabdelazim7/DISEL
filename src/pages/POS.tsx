...
export default function POS() {
  const { products, categories, cart, addToCart, addToCartQty, setCartLineQty, removeFromCart, updateQuantity, updatePrice, clearCart, checkout, processReturn, storeSettings, orders, activeInvoiceId, customers, activeCashier, logoutPOS, isOnline, offlineQueue, offlineReturnsQueue, isSyncing, syncOfflineQueue, syncOfflineReturnsQueue, addCashierNote, addExpense, invoiceType, setInvoiceType, employees, updateCartItemSalesperson, deleteOrder, savingsTransfer, addEmployeeTransaction, updateProduct, heldInvoices, holdInvoice, confirmHeldInvoice, returnHeldInvoice, saveQuotation, addCustomer, createInstallmentPlan } = useStore();
  ...
  // الموظف المختار للمنتج اللي بالوزن — بيتحدد قبل نافذة الوزن ويتخزن لحد التأكيد
  const [weightSalesperson, setWeightSalesperson] = useState<{ id: string; name: string } | null>(null);
  // لو مليان = بنعدّل سطر موجود (ضبط الكمية)، لو null = إضافة سطر جديد
  const [weightEditLineId, setWeightEditLineId] = useState<string | null>(null);

  // نافذة اختيار الموظف المسؤول: بتفتح مرة عند إضافة أي صنف (mode='add')
  // أو لتغيير موظف سطر موجود في السلة (mode='change').
  const [spPicker, setSpPicker] = useState<
    | { mode: 'add'; product: Product; qty: number }
    | { mode: 'change'; lineId: string; itemName: string; currentId?: string | null }
    | null
  >(null);

  // الموظفون غير النشطين ما ينفعش تتنسب لهم مبيعات جديدة.
  const activeEmployees = useMemo(() => employees.filter((e) => e.is_active !== false), [employees]);

  // كل صنف لازم يتحدد له الموظف المسؤول وقت الإضافة — عشان كده أي إضافة
  // بتعدّي على نافذة الاختيار الأول.
  const handleAddProduct = (product: Product, qty = 1) => {
    if (activeEmployees.length === 0) {
      alert('لا يوجد موظفون نشطون — أضِفهم من قسم الموظفين أولاً لتحديد المسؤول عن كل مبيعة.');
      return;
    }
    setSpPicker({ mode: 'add', product, qty });
  };

  // اتخذ القرار: كمّل الإضافة أو غيّر موظف سطر قائم
  const pickSalesperson = (emp: { id: string; name: string }) => {
    if (!spPicker) return;
    if (spPicker.mode === 'change') {
      updateCartItemSalesperson(spPicker.lineId, emp);
      setSpPicker(null);
      return;
    }
    const { product, qty } = spPicker;
    setSpPicker(null);
    if (isFractionalUnit(product.unit)) {
      // المنتجات بالوزن: الموظف اتحدد، فاضل الكمية
      setWeightProduct(product);
      setWeightEditLineId(null);
      setWeightSalesperson(emp);
      setWeightUnitInput('');
      setWeightSubInput('');
      return;
    }
    for (let i = 0; i < Math.max(1, qty); i++) addToCart(product, emp);
  };
  ...
  const confirmWeight = () => {
    ...
    if (qty <= 0) return;
    if (weightEditLineId) {
      // تعديل سطر موجود: بنضبط الكمية على القيمة المكتوبة. (كانت بتتجمع على
      // الكمية الحالية، فالنافذة تفتح على 2.5 وتكتب 3 فتطلع 5.5 والعميل يتحاسب غلط.)
      // بيشتغل حتى لو السطر بلا مسؤول (فاتورة معلقة قديمة) — التأكيد بيتمنع
      // من زرار الدفع لحد ما المسؤول يتحدد.
      setCartLineQty(weightEditLineId, qty);
    } else {
      if (!weightSalesperson) return;
      addToCartQty(weightProduct, qty, weightSalesperson);
    }
    setWeightProduct(null);
    setWeightSalesperson(null);
    setWeightEditLineId(null);
    setWeightUnitInput('');
    setWeightSubInput('');
  };
  ...
  const handleConfirmScanAdd = () => {
    if (scannedProduct) {
      // نفس مسار الإضافة العادي: اختيار الموظف الأول، وبعدين الوزن لو محتاج
      handleAddProduct(scannedProduct as Product, scanQty);
  ...
  // مفتاح سطر المرتجع: id صف order_items لو موجود، وإلا نرجع للمنتج+الموظف.
  const returnLineKey = (item: any) => item.order_item_id || cartLineId(item.id, item.salesperson_id);
  ...
  const handleConfirmReturns = async () => {
    ...
    // بنفتّش بمفتاح السطر مش بالمنتج — نفس المنتج ممكن يكون في سطرين
    // (موظفين مختلفين) وساعتها التفتيش بالمنتج بيرجّع أول سطر بس.
    const selected = Object.keys(pendingReturns).map(lineKey => {
       const pr = pendingReturns[lineKey];
       const item = activeReturnOrder.items.find((i: any) => returnLineKey(i) === lineKey);
       const effectivePrice = item ? item.sale_price * dr : 0;
       return {
         productId: item?.id as string,
         orderItemId: item?.order_item_id as string | undefined,
         returnQty: pr.returnQty || 0,
         itemValue: (pr.returnQty || 0) * effectivePrice,
       };
    }).filter(r => r.returnQty > 0 && r.productId);
...
<script>window.onload=()=>{setTimeout(()=>{window.print();window.onafterprint=()=>window.close();},500);}<\/script>
  ...
  const handleCheckoutClick = (shouldPrint: boolean) => {
    if (cart.length === 0) return;
    // فاتورة فيها صنف بلا مسؤول = مبيعات وعمولة ضايعة، وتصليحها بعد كده
    // محتاج دخول على مبيعات اليوم. أرخص نمنعها هنا.
    if (unassignedCount > 0) {
      alert(`فيه ${unassignedCount} صنف من غير موظف مسؤول. دوس على الصنف في السلة واختَر الكابتن قبل إتمام الفاتورة.`);
      return;
    }
  ...
  const doCheckout = async (shouldPrint: boolean) => {
    ...
    const currentSalesperson = cartSalespeople.map((s) => s.name).join('، '); // قبل ما الـ checkout يفضّي السلة
  ...
  // نسبة عمولة الخدمات — إعداد عام من لوحة الأدمن
  const serviceCommissionRate = storeSettings.serviceCommissionRate ?? DEFAULT_SERVICE_COMMISSION_RATE;

  // أصناف لسه بلا مسؤول — بتمنع إتمام الفاتورة عشان ما تضيعش عمولة حد.
  // بلا useMemo: السلة صغيرة، والـ Map اللي كانت جواه بتتعدّل مكانها فـ React
  // Compiler كان بيلغي تحسين الصفحة كلها.
  const unassignedCount = cart.filter((i) => !i.salesperson_id).length;

  // الكباتن في السلة + عدد الأسطر لكل واحد (للعرض في الفوتر)
  const cartSalespeople = cart.reduce<{ id: string; name: string; lines: number }[]>((acc, item) => {
    if (!item.salesperson_id) return acc;
    const found = acc.find((s) => s.id === item.salesperson_id);
    if (found) return acc.map((s) => (s.id === item.salesperson_id ? { ...s, lines: s.lines + 1 } : s));
    return [...acc, { id: item.salesperson_id, name: item.salesperson_name || '', lines: 1 }];
  }, []);
  ...
  const handleReturnAll = async () => {
    ...
    const returnsArray = activeReturnOrder.items.map((item: any) => {
      ...
      return {
        productId: item.id,
        // من غيره، لو المنتج في سطرين، الاتنين بيتحدّدوا بالمنتج فيروحوا على
        // أول سطر بس — والسطر التاني يفضل قابل للإرجاع تاني ويترد مرتين.
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-slate-900 transition-colors duration-300 overflow-hidden font-sans text-gray-900 dark:text-gray-100">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col border border-gray-200 dark:border-slate-700">
            ...
            <div className="p-6 flex-1 flex flex-col gap-4">
              ...
              {activeReturnOrder && (() => {
                ...
                const selectedReturnValue = Object.keys(pendingReturns).reduce((s, key) => {
                  const pr = pendingReturns[key];
                  const it = activeReturnOrder.items.find((i: any) => returnLineKey(i) === key);
                    ...
                    <div className="flex-1 border border-gray-200 dark:border-slate-700 flex flex-col rounded-xl overflow-hidden">
                      ...
                      <div className="p-4 space-y-3 max-h-72 overflow-y-auto hide-scrollbar">
                        {activeReturnOrder.items.map((item: any) => {
                          ...
                          // مفتاح السطر مش المنتج: لو نفس المنتج في سطرين، المفتاح
                          // بالمنتج بيخلي الصفين يشاركوا نفس الكمية ويتعدّلوا مع بعض.
                          const rowKey = returnLineKey(item);
                          const pr = pendingReturns[rowKey] || { returnQty: 0, refundAmount: 0, returnType: 'cash' };

                          return (
                            <div key={rowKey} className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-600 rounded-xl shadow-sm hover:shadow-md transition-shadow gap-4">
                                  ...
                                  <input 
                                    ...
                                    onChange={(e) => {
                                      ...
                                      setPendingReturns(prev => ({
                                        ...
                                        [rowKey]: {
                                          returnQty: qty,
                                          refundAmount: prev[rowKey]?.returnType === 'debt' ? 0 : qty * effectivePrice,
                                          returnType: prev[rowKey]?.returnType || 'cash'
                                        }
                                      }));
                                    }}
      ...
      <div className={`w-full pb-16 lg:pb-0 lg:w-1/3 min-w-0 lg:min-w-[320px] xl:min-w-[420px] bg-white dark:bg-slate-800 flex flex-col z-20 shadow-2xl relative ${mobileView === 'catalog' ? 'hidden lg:flex' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2 bg-slate-50 dark:bg-slate-900/50" style={{ scrollbarWidth: 'thin' }}>
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item.line_id} className="bg-white dark:bg-slate-800 p-3 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col gap-2 relative overflow-hidden group hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start">
                  <h4 className="font-bold text-gray-800 dark:text-gray-100 leading-tight w-4/5 text-sm">{item.name}</h4>
                  <button onClick={() => removeFromCart(item.line_id!)} aria-label="حذف الصنف" className="text-red-500 hover:text-white hover:bg-red-500 dark:text-red-400 transition-colors bg-red-50 dark:bg-red-900/30 p-2 rounded-lg absolute left-3 top-3 border border-red-100 dark:border-red-900/50 shadow-sm">
                    <Trash2 size={16} />
                  </button>
                </div>
                {/* الموظف المسؤول عن الصنف ده تحديداً — دوس عليه لتغييره */}
                <button
                  type="button"
                  onClick={() => setSpPicker({ mode: 'change', lineId: item.line_id!, itemName: item.name, currentId: item.salesperson_id })}
                  className={`flex items-center gap-1.5 self-start px-2 py-1 rounded-lg text-[11px] font-bold border transition-colors ${
                    item.salesperson_id
                      ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 border-emerald-100 dark:border-emerald-900/50 hover:bg-emerald-100'
                      : 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-300 border-red-200 dark:border-red-900/50 hover:bg-red-100 animate-pulse'
                  }`}
                >
                  <UserCheck size={12} />
                  {item.salesperson_name || 'اختر المسؤول'}
                  {(item.type === 'service') && item.salesperson_id ? (
                    <span className="text-[9px] opacity-70">• عمولة {serviceCommissionRate}%</span>
                  ) : null}
                </button>
                <div className="flex items-center justify-between pt-2 mt-0.5 border-t border-gray-50 dark:border-slate-700/50">
                      ...
                      <>
                        <div className="flex items-center gap-1.5 mb-0.5">
                          ...
                          <input
                            ...
                            onChange={(e) => updatePrice(item.line_id!, parseFloat(e.target.value) || 0)}
                  ...
                  {isFractionalUnit(item.unit) ? (
                    <button
                      onClick={() => { setWeightProduct(item); setWeightEditLineId(item.line_id!); setWeightSalesperson(item.salesperson_id ? { id: item.salesperson_id, name: item.salesperson_name || '' } : null); setWeightUnitInput(String(item.quantity)); setWeightSubInput(''); }}
                    ...
                    <div className="flex items-center bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-lg p-0.5 shadow-inner">
                      <button onClick={() => updateQuantity(item.line_id!, item.quantity - 1)} className="p-1.5 hover:bg-white dark:hover:bg-slate-600 rounded-md text-gray-600 dark:text-gray-300 transition-colors shadow-sm">
        ...
        <div className="p-3 bg-white dark:bg-slate-800 border-t border-gray-100 dark:border-slate-700 shadow-2xl">
          {/* ملخّص الكباتن — كل صنف له مسؤوله، فده عرض بس (التغيير من على الصنف نفسه) */}
          {cart.length > 0 && (
            <div className="mb-2" id="pos-salesperson">
              {unassignedCount > 0 ? (
                <div className="flex items-center gap-1.5 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/50 rounded-lg px-2.5 py-1.5 text-[11px] font-bold text-red-600 dark:text-red-300">
                  <UserCheck size={13} />
                  {unassignedCount} صنف بدون مسؤول — دوس على الصنف واختَر الكابتن.
                </div>
              ) : (
                <div className="flex flex-wrap items-center gap-1.5">
                  <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400">👤 الكباتن:</span>
                  {cartSalespeople.map((s) => (
                    <span key={s.id} className="bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 border border-emerald-100 dark:border-emerald-900/50 rounded-lg px-2 py-0.5 text-[11px] font-bold">
                      {s.name} <span className="opacity-60">({s.lines})</span>
                    </span>
                  ))}
                </div>
              )}
      ...
      {/* ── نافذة اختيار الموظف المسؤول عن الصنف ── */}
      {/* بتفتح مع كل إضافة: دوسة واحدة على اسم الكابتن = اختيار + إضافة. */}
      {spPicker && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[320] flex items-center justify-center p-4" onClick={() => setSpPicker(null)}>
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md animate-in zoom-in-95 flex flex-col max-h-[85vh]" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between p-5 border-b border-gray-100 dark:border-slate-700">
              <div className="min-w-0">
                <h3 className="font-black text-lg text-slate-800 dark:text-white leading-tight flex items-center gap-2">
                  <UserCheck size={20} className="text-indigo-600 shrink-0" />
                  {spPicker.mode === 'change' ? 'تغيير المسؤول' : 'مين المسؤول؟'}
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 font-bold mt-1 truncate">
                  {spPicker.mode === 'change' ? spPicker.itemName : spPicker.product.name}
                  {spPicker.mode === 'add' && spPicker.qty > 1 ? ` × ${spPicker.qty}` : ''}
                </p>
              </div>
              <button onClick={() => setSpPicker(null)} aria-label="إغلاق" className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1 shrink-0">
                <X size={22} />
              </button>
            </div>
            ...
            <div className="p-4 overflow-y-auto grid grid-cols-2 gap-2">
              {activeEmployees.map((emp) => {
                const isCurrent = spPicker.mode === 'change' && spPicker.currentId === emp.id;
                return (
                  <button
                    type="button"
                    key={emp.id}
                    onClick={() => pickSalesperson({ id: emp.id, name: emp.name })}
                    className={`px-3 py-3 rounded-2xl text-sm font-bold border text-right transition ${
                      isCurrent
                        ? 'bg-indigo-600 text-white border-transparent shadow'
                        : 'bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:border-indigo-300 active:scale-95'
                    }`}
                  >
                    <span className="block truncate">{isCurrent ? '✓ ' : ''}{emp.name}</span>
                    {emp.job_title ? <span className="block text-[10px] opacity-60 font-bold truncate">{emp.job_title}</span> : null}
                  </button>
                );
              })}
            </div>
      ...
      {weightProduct && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[300] flex items-center justify-center p-4" onClick={() => { setWeightProduct(null); setWeightEditLineId(null); setWeightSalesperson(null); }}>
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-sm animate-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-start justify-between p-5 border-b border-gray-100 dark:border-slate-700">
                ...
                <button onClick={() => { setWeightProduct(null); setWeightEditLineId(null); setWeightSalesperson(null); }} className="text-slate-400 hover:text-slate-600 bg-slate-50 dark:bg-slate-700 p-2 rounded-xl"><X size={18} /></button>
              </div>
