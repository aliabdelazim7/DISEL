...
import { cartLineId, cartQtyOfProduct, cartQtyByProduct } from '../utils/cart';
...
export interface OrderItem extends Product {
  ...
  // مفتاح السطر في السلة = المنتج + الموظف المسؤول. نفس المنتج مع موظفين
  // مختلفين = سطرين منفصلين، عشان كده مينفعش نفتّش بـ id لوحده.
  line_id?: string;
  // id صف order_items نفسه (مش المنتج) — لازم لتعديل العمولة من مبيعات اليوم.
  order_item_id?: string;
  salesperson_id?: string | null;
  salesperson_name?: string | null;
  commission_amount?: number; // عمولة المنتج اليدوية لكل قطعة (بالجنيه)
  commission_confirmed?: boolean;
}

// الدوال دي اتنقلت لـ utils/cart.ts (دوال خالصة بلا supabase عشان تتختبر).
// بنعيد تصديرها من هنا عشان الاستيرادات القديمة تفضل شغالة.
export { cartLineId, cartQtyOfProduct, cartQtyByProduct } from '../utils/cart';
...
export interface Order {
  ...
  // قائمة الكباتن المنفّذين — مشتقّة من أصناف الفاتورة (بدون تكرار) وبتتخزّن
  // للفاتورة المطبوعة والتقارير القديمة. حساب المبيعات والعمولة بيتم من
  // order_items.salesperson_id لكل صنف، مش من هنا.
...
export interface HeldInvoiceItem {
  ...
  category_id?: string;
  type?: string;
  // الموظف المسؤول محفوظ جوّه الصنف نفسه — عمود salesperson_id على الفاتورة
  // المعلقة بيحفظ كابتن واحد بس، فلو اعتمدنا عليه الفاتورة اللي فيها أكتر من
  // كابتن هترجع كلها منسوبة لواحد.
  salesperson_id?: string | null;
  salesperson_name?: string | null;
}
...
export interface StoreSettings {
  ...
  serviceCommissionRate?: number; // نسبة عمولة الخدمات % للموظف المنفّذ (افتراضياً 10)
}
...
interface CashierStore {
  ...
  // Cart
  addToCart: (product: Product, salesperson: { id: string; name: string }) => void;
  addToCartQty: (product: Product, quantity: number, salesperson: { id: string; name: string }) => void;
  setCartLineQty: (lineId: string, quantity: number) => void;
  removeFromCart: (lineId: string) => void;
  updateQuantity: (lineId: string, quantity: number) => void;
  updatePrice: (lineId: string, price: number) => void;
  updateCartItemSalesperson: (lineId: string, salesperson: { id: string; name: string }) => void;
  clearCart: () => void;
  ...
  processReturn: (orderId: string, returns: { productId: string, orderItemId?: string, returnQty: number, refundAmount: number, debtDeduction?: number }[], refundMethod?: string) => Promise<boolean>;
  ...
  // عمولة المنتج اليدوية لكل صنف في الفاتورة (من صفحة مبيعات اليوم)
  updateItemCommission: (orderItemId: string, commissionAmount: number, confirmed: boolean) => Promise<boolean>;
  // تعديل الموظف المسؤول عن صنف اتباع قبل كده (فواتير قديمة أو تصحيح غلطة)
  updateItemSalesperson: (orderItemId: string, salesperson: { id: string; name: string }) => Promise<boolean>;
...
// ─── Helpers ─────────────────────────────────────────────────
// نسبة عمولة الخدمات الافتراضية % — تتغيّر من الإعدادات (store_settings.service_commission_rate)
export const DEFAULT_SERVICE_COMMISSION_RATE = 10;
...
function mapSettings(row: Record<string, unknown>): StoreSettings {
  return {
    ...
    serviceCommissionRate: (row.service_commission_rate as number) ?? DEFAULT_SERVICE_COMMISSION_RATE,
  };
}
...
export const useStore = create<CashierStore>((set, get) => ({
  storeSettings: {
    ...
    serviceCommissionRate: DEFAULT_SERVICE_COMMISSION_RATE,
  },
  products: [],
  categories: [],
  customers: [],
      ...
      const orders: Order[] = ((ordersRes.data ?? []) as Record<string, unknown>[]).map((o) => {
        ...
        const items: OrderItem[] = itemRows.map((i) => {
          ...
          return {
            ...
            // نوع الصنف بيحدد نوع العمولة (خدمة = نسبة % / منتج = قيمة يدوية)،
            // فلازم يتحمّل مع الفاتورة وإلا كل الأصناف هتتحسب منتجات.
            // النوع المتجمّد وقت البيع هو المرجع؛ products بيتغيّر أو يتمسح.
            type: (i.item_type as string) ?? (prod.type as string) ?? 'product',
            ...
            salesperson_id: (i.salesperson_id as string) ?? null,
            salesperson_name: (i.salesperson_name as string) ?? null,
            commission_amount: (i.commission_amount as number) ?? 0,
            commission_confirmed: Boolean(i.commission_confirmed),
          };
        });
        return {
          id: o.id as string,
    ...
    for (const offlineOrder of queue) {
      try {
        ...
        const itemsPayload = offlineOrder.items.map((item: any) => ({
          ...
          // من غير دول، أي فاتورة اتعملت أوفلاين هتترفع بلا موظف مسؤول
          // وتضيع عمولتها.
          salesperson_id: item.salesperson_id || null,
          salesperson_name: item.salesperson_name || null,
  ...
  // ── Cart ───────────────────────────────────────────────────
  addToCart: (product, salesperson) =>
    set((state) => {
      if (product.stock_quantity <= 0) return state;
      const step = unitStep(product.unit); // 1 للقطعة، 0.25 للوحدات الكسرية
      const lineId = cartLineId(product.id, salesperson.id);
      const existing = state.cart.find((i) => i.line_id === lineId);
      // المخزون بيتحسب على مجموع أسطر المنتج كلها، مش على السطر الواحد —
      // نفس المنتج ممكن يبقى في أكتر من سطر بموظفين مختلفين، ولو كل سطر
      // اتقاس لوحده هينفع نبيع ضعف المخزون.
      const otherLinesQty = cartQtyOfProduct(state.cart, product.id, lineId);
      const available = product.stock_quantity - otherLinesQty;
      if (available <= 0) return state;
      if (existing) {
        if (existing.quantity >= available) return state;
        const next = Math.min(existing.quantity + step, available);
        return { cart: state.cart.map((i) => (i.line_id === lineId ? { ...i, quantity: next } : i)) };
      }
      const first = Math.min(step, available);
      const price = priceForType(product, state.invoiceType);
      return {
        cart: [...state.cart, {
          ...product, sale_price: price, quantity: first, returned_quantity: 0,
          line_id: lineId, salesperson_id: salesperson.id, salesperson_name: salesperson.name,
        }],
      };
    }),

  // إضافة منتج للسلة بكمية محددة (تُستخدم لإدخال الوزن من شاشة الكاشير)
  addToCartQty: (product, quantity, salesperson) =>
    set((state) => {
      ...
      if (available <= 0) return state;
      if (existing) {
        const next = Math.max(min, Math.min(existing.quantity + quantity, available));
        return { cart: state.cart.map((i) => (i.line_id === lineId ? { ...i, quantity: next } : i)) };
      }
      const qty = Math.max(min, Math.min(quantity, available));
      const price = priceForType(product, state.invoiceType);
      return {
        cart: [...state.cart, {
          ...product, sale_price: price, quantity: qty, returned_quantity: 0,
          line_id: lineId, salesperson_id: salesperson.id, salesperson_name: salesperson.name,
        }],
      };
    }),

  // ضبط كمية سطر بالوزن لقيمة محددة (تعديل، مش إضافة)
  setCartLineQty: (lineId, quantity) =>
    set((state) => {
      const item = state.cart.find((i) => i.line_id === lineId);
      if (!item || quantity <= 0) return state;
      const product = state.products.find((p) => p.id === item.id);
      const stock = product?.stock_quantity ?? item.stock_quantity;
      const available = stock - cartQtyOfProduct(state.cart, item.id, lineId);
      const min = unitMinQty(item.unit);
      const next = Math.max(min, Math.min(quantity, available));
      return { cart: state.cart.map((i) => (i.line_id === lineId ? { ...i, quantity: next } : i)) };
    }),

  removeFromCart: (lineId) => set((state) => ({ cart: state.cart.filter((i) => i.line_id !== lineId) })),
  ...
  updateQuantity: (lineId: string, quantity: number) =>
    set((state) => {
      ...
      // المتاح = المخزون ناقص الأسطر التانية لنفس المنتج
      const available = product.stock_quantity - cartQtyOfProduct(state.cart, item.id, lineId);
      const validQty = Math.max(unitMinQty(product.unit), Math.min(quantity, available));
      return { cart: state.cart.map((i) => (i.line_id === lineId ? { ...i, quantity: validQty } : i)) };
    }),

  updatePrice: (lineId, price) =>
    set((state) => ({
      cart: state.cart.map((i) => (i.line_id === lineId ? { ...i, sale_price: price } : i))
    })),

  // تغيير الموظف المسؤول عن سطر في السلة. لو الموظف الجديد عنده نفس المنتج
  // في سطر تاني، بندمج الكميتين بدل ما نسيب سطرين بنفس المفتاح.
  updateCartItemSalesperson: (lineId, salesperson) =>
    set((state) => {
      const item = state.cart.find((i) => i.line_id === lineId);
      if (!item || item.salesperson_id === salesperson.id) return state;
      const newLineId = cartLineId(item.id, salesperson.id);
      const target = state.cart.find((i) => i.line_id === newLineId);
      if (target) {
        // مجموع السطرين أصلاً مش بيعدّي المخزون (addToCart بيقيس على المجموع)،
        // فالدمج بيجمع الكميتين من غير ما يقصّ حاجة ويضيّع كمية من الفاتورة.
        const merged = target.quantity + item.quantity;
        return {
          cart: state.cart
            .filter((i) => i.line_id !== lineId)
            .map((i) => (i.line_id === newLineId ? { ...i, quantity: merged } : i)),
        };
      }
      return {
        cart: state.cart.map((i) => (i.line_id === lineId
          ? { ...i, line_id: newLineId, salesperson_id: salesperson.id, salesperson_name: salesperson.name }
          : i)),
      };
    }),
  ...
  checkout: async (total, customerDetails, paidAmount = total, type = 'sale', paymentMethod = 'cash', splitPayments, cashierName, notes, couponCode, discountAmount, carId) => {
    ...
    // الكباتن دلوقتي بيتحددوا لكل صنف، فقائمة الفاتورة بتتشتق من الأصناف
    // (بدون تكرار) — بتفضل موجودة عشان الفاتورة المطبوعة والتقارير القديمة،
    // لكن حساب المبيعات والعمولة بقى من order_items.salesperson_id.
    const salespeople = state.cart.reduce<{ id: string; name: string }[]>((acc, item) => {
      if (item.salesperson_id && !acc.some((s) => s.id === item.salesperson_id)) {
        acc.push({ id: item.salesperson_id, name: item.salesperson_name || '' });
      }
      return acc;
    }, []);
    ...
    const executeOfflineCheckout = () => {
      ...
      const cartQty = cartQtyByProduct(state.cart);
      const updatedProducts = state.products.map((p) => {
        const qty = cartQty.get(p.id);
        return qty ? { ...p, stock_quantity: Math.max(0, p.stock_quantity - qty) } : p;
      });
    ...
    try {
      ...
      // بنرجّع id كل صف عشان الحالة المحلية تعرفه. من غيره، «مبيعات اليوم»
      // تحاول تحفظ عمولة فاتورة اتعملت في نفس الجلسة على id مش موجود
      // فيرجع خطأ uuid، والأدمن ما يقدرش يأكّد غير بعد ما يعمل refresh.
      const cartWithIds = state.cart.map((i, idx) => ({
        ...i,
        order_item_id: (insertedItems?.[idx] as any)?.id as string | undefined,
      }));

      // Update stock
      // مجمّعة لكل منتج: المنتج ممكن يكون في أكتر من سطر (موظفين مختلفين)،
      // ولو خصمنا سطر سطر التحديث التاني بيقرا نفس المخزون القديم ويكتب فوق الأول.
      for (const [productId, qty] of cartQtyByProduct(state.cart)) {
        const newQty = (state.products.find((p) => p.id === productId)?.stock_quantity ?? 0) - qty;
        await supabase.from('products').update({ stock_quantity: Math.max(0, newQty) }).eq('id', productId);
      }

      // Build new order for local state
      const newOrder: Order = {
        id: invoiceId,
        items: cartWithIds.map((i) => ({ ...i })),
        total,
        paid_amount: savedPaidAmount,
        paid_cash: splits.cash,
        paid_visa: splits.visa,
  ...
  holdInvoice: async ({ customerName, customerPhone, customerCustomId, notes } = {}) => {
    ...
    try {
      ...
      // العمود المفرد salesperson_id بيتساب للتوافق (أول كابتن)، والتوزيع
      // الحقيقي محفوظ لكل صنف جوّه items.
      const sp = state.cart.find((i) => i.salesperson_id)
        ? { id: state.cart.find((i) => i.salesperson_id)!.salesperson_id!, name: state.cart.find((i) => i.salesperson_id)!.salesperson_name || '' }
        : null;
      ...
      const items: HeldInvoiceItem[] = state.cart.map((i) => ({
        ...
        type: i.type,
        salesperson_id: i.salesperson_id || null,
        salesperson_name: i.salesperson_name || null,
      }));
  ...
  confirmHeldInvoice: async (id) => {
    ...
    try {
      ...
      // مجمّعة لكل منتج: .find() بترجع أول سطر بس، فلو المنتج في سطرين
      // (موظفين مختلفين) الحالة المحلية بترجّع كمية سطر واحد بينما الداتابيز
      // بترجّع الاتنين — والفرق ده بيتخصم غلط من المخزون في أول فاتورة جاية.
      const restoredQty = new Map<string, number>();
      for (const it of held.items) restoredQty.set(it.id, (restoredQty.get(it.id) || 0) + (Number(it.quantity) || 0));
      const restoredProducts = state.products.map((p) => {
        const qty = restoredQty.get(p.id);
        return qty ? { ...p, stock_quantity: p.stock_quantity + qty } : p;
      });
      ...
      const cartItems: OrderItem[] = held.items.map((it) => {
        ...
        // الفواتير المعلقة القديمة (اتحفظت قبل الموظف لكل صنف) بترجع بكابتن
        // الفاتورة على كل الأصناف — أحسن من ما ترجع بلا موظف خالص.
        const spId = it.salesperson_id ?? held.salesperson_id ?? null;
        const spName = it.salesperson_name ?? held.salesperson_name ?? null;
        return {
          ...base, sale_price: it.sale_price, quantity: it.quantity, returned_quantity: 0,
          line_id: cartLineId(it.id, spId), salesperson_id: spId, salesperson_name: spName,
        };
      });
  ...
  processReturn: async (orderId, returns, refundMethod = 'cash') => {
    ...
    const executeOfflineReturn = () => {
      ...
      for (const ret of returns) {
        // بالسطر مش بالمنتج: لو المنتج في سطرين (موظفين مختلفين) الماتش
        // بالمنتج بيعلّم الاتنين مرتجعين ويحسب ضعف المبلغ المردود.
        const idx = ret.orderItemId
          ? updatedItems.findIndex((i) => i.order_item_id === ret.orderItemId)
          : updatedItems.findIndex((i) => i.id === ret.productId);
        if (idx === -1) continue;
        updatedItems = updatedItems.map((i, k) =>
          k === idx ? { ...i, returned_quantity: i.returned_quantity + ret.returnQty, refunded_amount: (i.refunded_amount || 0) + ret.refundAmount } : i
        );
      ...
      if (orderId.startsWith('OFF-')) {
        const updatedQueue = state.offlineQueue.map((o) => {
          if (o.id === orderId) {
            let oItems = [...o.items];
            for (const ret of returns) {
              // نفس الحكاية: سطر واحد بالظبط
              const idx = ret.orderItemId
                ? oItems.findIndex((i: any) => i.order_item_id === ret.orderItemId)
                : oItems.findIndex((i: any) => i.id === ret.productId);
              if (idx === -1) continue;
              oItems = oItems.map((i: any, k: number) =>
                k === idx ? { ...i, returned_quantity: (i.returned_quantity || 0) + ret.returnQty, refunded_amount: (i.refunded_amount || 0) + ret.refundAmount } : i
              );
            }
            return {
              ...o,
    ...
    try {
      ...
      for (const ret of returns) {
        // بنحدد السطر بـ order_item_id لو متبعت. التفتيش بالمنتج بقى غير كافي:
        // نفس المنتج ممكن يكون في سطرين (موظفين مختلفين) و .single() بترمي
        // خطأ لما تلاقي صفين — وكان الخطأ بيتبلع في صمت، يعني الفلوس بترجع
        // للعميل والمخزون بيزيد من غير ما المرتجع يتسجّل في الداتابيز أصلاً.
        const itemIndex = ret.orderItemId
          ? updatedItems.findIndex(i => i.order_item_id === ret.orderItemId)
          : updatedItems.findIndex(i => i.id === ret.productId);
        ...
        let orderItemId = ret.orderItemId || item.order_item_id;
        if (!orderItemId) {
          // فواتير قديمة اتحمّلت من غير id الصف — بناخد أول صف مطابق.
          ...
          orderItemId = (rows?.[0] as Record<string, unknown> | undefined)?.id as string | undefined;
        }
        if (!orderItemId) throw new Error(`تعذّر تحديد صنف المرتجع في الفاتورة ${orderId}`);
        ...
        // بالفهرس عشان ما نعدّلش أسطر تانية لنفس المنتج
        updatedItems = updatedItems.map((i, idx) =>
          idx === itemIndex ? { ...i, returned_quantity: newReturnedQty, refunded_amount: newRefundedAmt } : i
        );
      }
  ...
  editOrder: async (orderId, updatedData, updatedItems, reason) => {
    ...
    try {
      ...
      const itemsPayload = updatedItems.map((item) => ({
        ...
        sale_price: item.sale_price,
        purchase_price: item.average_purchase_price || item.purchase_price,
        // تعديل الفاتورة بيمسح الأصناف ويعيد إدخالها، فلو ما نقلناش دول
        // الموظف المسؤول والعمولة المؤكَّدة بيتمسحوا وتضيع مستحقاته.
        salesperson_id: item.salesperson_id || null,
        salesperson_name: item.salesperson_name || null,
        item_type: item.type || 'product',
        commission_amount: item.commission_amount || 0,
        commission_confirmed: item.commission_confirmed || false,
      }));
      ...
      // الأصناف المعاد إدخالها لها id جديد. من غير ما نحدّث order_item_id محلياً،
      // «مبيعات اليوم» هتحاول تعدّل عمولة صف اتمسح — من غير خطأ، بس من غير ما
      // يتحفظ حاجة. الترتيب هنا هو نفس ترتيب itemsPayload.
      const itemsWithIds = (insertedItems && insertedItems.length === updatedItems.length)
        ? updatedItems.map((it, idx) => ({ ...it, order_item_id: (insertedItems[idx] as any).id as string }))
        : updatedItems;
    ...
    for (const returnBatch of queue) {
      try {
        ...
        for (const returnItem of batchReturns) {
          // بالـ id لو متسجّل وقت المرتجع الأوفلاين، وإلا بالمنتج (فواتير قديمة).
          // .single() على المنتج بترمي خطأ لو المنتج في سطرين بموظفين مختلفين.
          const baseQuery = supabase
            .from('order_items')
            .select('id, returned_quantity, refunded_amount');
          const orderItemRow = returnItem.orderItemId
            ? await baseQuery.eq('id', returnItem.orderItemId).maybeSingle()
            : await baseQuery.eq('order_id', batchOrderId).eq('product_id', returnItem.productId).limit(1).maybeSingle();
  ...
  updateSettings: async (newSettings) => {
    ...
    if (newSettings.serviceCommissionRate !== undefined) mapped.service_commission_rate = newSettings.serviceCommissionRate;
  ...
  // ── عمولات الأصناف (صفحة مبيعات اليوم) ──────────────────────
  updateItemCommission: async (orderItemId, commissionAmount, confirmed) => {
    ...
    if (error) {
      console.error('Update item commission error:', error);
      alert('تعذّر حفظ العمولة: ' + error.message);
      return false;
    }
    ...
    set((state) => ({
      orders: state.orders.map((o) => ({
        ...o,
        items: o.items.map((it) => (it.order_item_id === orderItemId
          ? { ...it, commission_amount: amount, commission_confirmed: confirmed }
          : it)),
      })),
    }));
    return true;
  ...
  updateItemSalesperson: async (orderItemId, salesperson) => {
    const { error } = await supabase
      .from('order_items')
      .update({ salesperson_id: salesperson.id, salesperson_name: salesperson.name })
      .eq('id', orderItemId);

    if (error) {
      console.error('Update item salesperson error:', error);
      alert('تعذّر تغيير الموظف المسؤول: ' + error.message);
      return false;
    }
    ...
    set((state) => ({
      orders: state.orders.map((o) => ({
        ...o,
        items: o.items.map((it) => (it.order_item_id === orderItemId
          ? { ...it, salesperson_id: salesperson.id, salesperson_name: salesperson.name }
          : it)),
      })),
    }));
    return true;
...
setupRealtime: () => {
      ...
      .on(
        ...
        async (payload) => {
          ...
          const formattedOrder: Order = {
            ...
            // لازم يفضل متطابق مع الماب اللي في loadAll — من غير الحقول دي
            // الفاتورة الجاية لايف تظهر في «مبيعات اليوم» بلا مسؤول ولا نوع.
            items: (items || []).map(i => ({
              id: i.product_id,
              order_item_id: i.id,
              name: i.product_name,
              ...
              sale_price: i.sale_price,
              type: i.item_type || i.products?.type || 'product',
  ...
  deleteEmployee: async (id) => {
    // order_items.salesperson_id فيه on delete set null، فمسح موظف عليه مبيعات
    // بيصفّر نسبته من كل الشهور اللي فاتت (حتى المتقفلة) وأصنافه تبقى "بدون
    // مسؤول". الأرشيف ده بيتحاسب عليه فلوس، فبنمنع المسح ونرشّح إلغاء التنشيط.
    ...
    if (countError) {
      console.error('Check employee sales error:', countError);
      alert('تعذّر التأكد من مبيعات الموظف. حاول تاني.');
      return;
    }

    if ((count || 0) > 0) {
      alert(`الموظف ده مسجّل عليه ${count} صنف مبيعات — مسحه هيضيّع مبيعاته وعمولاته من كل الشهور.\nاعمله «غير نشط» من تعديل بياناته بدل ما تمسحه.`);
      return;
    }
    ...
    if (error) {
      ...
      alert('تعذّر حذف الموظف: ' + error.message);
      return;
    }
    set((state) => ({
